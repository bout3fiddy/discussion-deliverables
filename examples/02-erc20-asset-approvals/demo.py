...
def swap_requires_an_approval():
    ...
    print(
        f"  after approve, swap returned {dy // UNIT} and the user received {asset1.balanceOf(user) // UNIT} of asset1"
    )
    assert dy == 10 * UNIT
...
def swap_received_needs_no_approval():
    ...
    print(
        f"  user transferred 10 of asset0 directly; allowance is {asset0.allowance(user, pool.address)}"
    )
    dy = pool.swap_received(0, 1, user, sender=user)
    print(
        f"  swap_received returned {dy // UNIT} and the user received {asset1.balanceOf(user) // UNIT} of asset1"
    )
    assert dy == 10 * UNIT
    assert asset1.balanceOf(user) == 10 * UNIT
    print()
